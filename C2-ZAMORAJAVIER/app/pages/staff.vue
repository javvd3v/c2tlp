    ...
    async function eliminarEstudiante(run: string) {
        ...
        try {
            await $fetch(`/api/estudiantes/${run}`, {
                method: 'DELETE'
            })
            await refresh()
        } catch (err: any) {
            console.error('Error al eliminar estudiante:', err)
            alert(getApiErrorMessage(err, 'No se pudo eliminar el estudiante.'))
        }
    }
    ...
    async function inscribir(eventoId: number) {
        try {
            ...
            alert(getApiErrorMessage(err, 'No se pudo inscribir al evento.'))
        }
    }
        ...
        <div class="min-h-screen bg-linear-to-b from-blue-900 to-gray-900 py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                ...
                <UAlert v-if="error" color="error" variant="soft" class="rounded-2xl mb-6"
                    :title="error.statusMessage || 'Error al cargar los eventos'"
                ...
                <div v-else-if="eventos && eventos.length > 0"
                    ...
                    <div v-for="evento in eventos" :key="evento.id"
                        ...
                        <div class="p-6">
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100 text-gray-200 flex items-center justify-end">
                                <UButton color="primary" variant="outline" @click="eventoSeleccionado = evento"
                                    :disabled="cuposDisponibles(evento) <= 0">
                                    Seleccionar Evento
                                </UButton>
                            </div>
                        </div>
                ...
                <div v-if="eventoSeleccionado"
                    ...
                    <!-- Cargando -->
                    <div v-if="pending"
                        class="space-y-4 rounded-3xl border border-course-line bg-course-surface-soft p-6">
                        <div class="space-y-1">
                            <p class="text-sm font-semibold text-course-header">Cargando estudiantes...</p>
                            <p class="text-sm text-course-text-muted">Estamos actualizando la información de la tabla.
                            </p>
                        </div>
                    ...
                    <UAlert v-if="error" color="error" variant="soft"
                        ...
                        :title="error.statusMessage || 'Error al cargar los estudiantes'"
                        icon="i-heroicons-exclamation-triangle" />
                    ...
                    <div v-if="eventoSeleccionado.estudiantes && eventoSeleccionado.estudiantes.length > 0"
                        ...
                        <table class="min-w-full divide-y divide-course-line-soft">
                            <thead class="bg-course-surface-soft">
                                <tr>
                                    <th
                                        class="px-4 py-3 text-left text-xs font-medium text-course-text-muted uppercase tracking-wider">
                                        RUN</th>
                                    <th
                                        class="px-4 py-3 text-left text-xs font-medium text-course-text-muted uppercase tracking-wider">
                                        Nombres</th>
                            ...
                            <tbody class="divide-y divide-course-line-soft">
                                <tr v-for="estudiante in eventoSeleccionado.estudiantes" :key="estudiante.run">
                                    <td class="px-4 py-3 text-sm text-course-text">{{ estudiante.run }}</td>
                                    <td class="px-4 py-3 text-sm text-course-text">{{ estudiante.nombres }}</td>
                                    <td class="px-4 py-3 text-sm text-course-text">{{ estudiante.apellidos }}</td>
                                    <td class="px-4 py-3 text-sm text-course-text">{{ estudiante.email || '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-right text-course-text">
                                        <UButton color="error" variant="outline" icon="i-heroicons-trash" size="sm"
                                            @click="
                                                eliminarEstudiante(estudiante.run)">
                                            Eliminar
                                        </UButton>
                                    </td>
                                </tr>
            ...
            <form class="space-y-5" @submit.prevent="crearEvento">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <UFormField label="Cupos Máximos" required name="topePersonas">
                        <UInput v-model.number="nuevoEvento.topePersonas" type="number" min="0" placeholder="0"
                            required />
                    </UFormField>
                </div>
